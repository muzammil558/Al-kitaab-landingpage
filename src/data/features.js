...
export const fetchRecitation = async (id, chapterNumber) => {
  try {
    const response = await axios.get(
      `https://api.quran.com/api/v4/chapter_recitations/${id}/${chapterNumber}`,
      {
        headers: { "Content-Type": "application/json" },
      }
    );
    return response.data; // Ensure it contains `audio_url`
  } catch (error) {
    console.error("Error fetching recitation:", error);
    throw new Error('Something went wrongs!', error.message);// Handle error case gracefully
  }
};
export const fetchVerses = async (chapterNumber) => {
  try {
    const response = await axios.get(
      `https://api.quran.com/api/v4/verses/by_chapter/${chapterNumber}`,
      {
        headers: { "Content-Type": "application/json"  },
      }
    );
    return response.data; // Ensure it contains the data you're interested in
  } catch (error) {
    console.error("Error fetching verses:", error);
    throw new Error('Something went wrong while fetching verses!', error.message); // Handle error case gracefully
  }
};

export const name_chapter = async (chapterName) => {
  try {
    const response = await axios.get(
      `https://api.quran.com/api/v4/chapters/${chapterName}`,
      {
        headers: { "Content-Type": "application/json" }
      }
    );
    ...
    throw new Error(`Something went wrong while fetching Name! ${error.message}`);
  }
};
